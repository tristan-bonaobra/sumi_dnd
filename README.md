...
A full-quality PDF extract is downloaded from the Canva website for the program to manually snip and stitch. [read more](#dnd_ingestpdf)

### Manually? What if some text moves 2 pixels to the right?

There are built-in buffers to account for slight deviations and bleed (see `config.toml`). [read more](#dnd_ingestpdf)
...
No need to worry about that. This uses [**Llama 3.2**](https://ollama.com/library/llama3.2) **via** [**Ollama**](https://github.com/ollama/ollama) on your local machine by default. [read more](#dnd_ingesttag)
